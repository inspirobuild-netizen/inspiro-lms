...
type LeadRow = {
  ...
  ownerId: string | null; ownerName: string | null; nextFollowupAt: string | null; createdAt: string;
};
...
export default function LeadsPage() {
  ...
  const canAssign = has('leads.view_all') && has('leads.manage');
  ...
  const [appLeadsOnly, setAppLeadsOnly] = useState(false);
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['crm', 'leads', page, debounced, status, priority, appLeadsOnly],
    queryFn: () => {
      const p = new URLSearchParams({ page: String(page), limit: String(limit) });
      if (debounced) p.set('search', debounced);
      if (appLeadsOnly) {
        p.set('source', 'mobile_app');
  ...
  const staffQ = useQuery({
    ...
    enabled: !!accessToken && appLeadsOnly,
  });

  const assign = useMutation({
    mutationFn: ({ id, ownerId }: { id: string; ownerId: string }) => api.patch(`/api/v1/leads/${id}`, { ownerId }),
  ...
  const columns: Column<LeadRow>[] = [
    ...
    appLeadsOnly
      ? { key: 'assign', header: 'Assign to', width: 'w-48', render: (l) => (
          <Select
            defaultValue=""
            onChange={(e) => { if (e.target.value) assign.mutate({ id: l.id, ownerId: e.target.value }); }}
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">{data?.meta?.total ?? 0} leads{appLeadsOnly ? ' · unassigned app leads' : ''}</p>
        </div>
      ...
      <div className="flex flex-wrap gap-3 items-center">
        ...
        {canAssign && (
          <button
            onClick={() => { setAppLeadsOnly((v) => !v); setPage(1); }}
            className={`px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${
              appLeadsOnly ? 'bg-brand-violet text-white' : 'bg-surface-2 text-slate-400 hover:text-slate-200'
            }`}
          >
            📱 App leads (unassigned)
          </button>
        )}
      </div>

      <DataTable columns={columns} data={data?.data ?? []} loading={isLoading} getKey={(l) => l.id} emptyMessage={appLeadsOnly ? 'No unassigned app leads 🎉' : 'No leads found'} />
